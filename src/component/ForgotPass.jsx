...
const ForgotPass = () => {
  ...
  const next = () => {
    ...
    .then((values) => {
      ...
      axios.post(`http://localhost:8080/api/password/forgot?email=${values.email}`)
        .then(response => {
  ...
  const onFinish = (values) => {
    ...
    axios.post(`http://localhost:8080/api/password/reset?email=${email}&newPassword=${newPassword}&otp=${otp}`)
      .then(response => {
